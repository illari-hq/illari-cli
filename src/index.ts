...
PING OPTIONS
  -d, --data k=v  metric to send with the check-in (repeatable; numeric values)

API OPTIONS (list, import)
...
BEHAVIOUR
  ...
  list    Prints the account's monitors and their current state.
...
EXAMPLES
  ...
  illari ping --key abc123                          # bare check-in
  illari ping --key abc123 -d rows=$n -d cost_usd=$c
  ILLARI_TOKEN=illari_... illari list
  illari import --dry-run                           # preview from your crontab
`;
